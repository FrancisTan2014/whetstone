...
- **Do not edit curated materials, prompt templates, category definitions, or default settings.** These are server-resident data curated by the human Admin role (see COWORK.md). Agents do not tune the prompts that judge user content, do not edit the materials users read, and do not redefine the categories. If implementation requires reading these as data, fetch them through the proper interface; never hard-code them in source.
- **Do not commit an ADR (or any locked-decision change) without updating STABLE.md in the same commit.** The same-commit rule keeps STABLE.md trustworthy. Same rule applies to [REVIEW_SPEC.md](./REVIEW_SPEC.md): any change requires a paired ADR.
