...
**Mutation testing backs the mutation-resistance gate (advisory).** For the two pure, logic-dense packages `@whetstone/domain` and `@whetstone/contracts`, `pnpm mutation` (Stryker, config `stryker.conf.mjs`) automates the check above: it plants representative bugs and reports which survive the tests, objectively surfacing shallow/happy-path-only tests that still hit 100% line coverage. It is **advisory only** — a separate `pnpm mutation` script and a nightly CI job (`.github/workflows/mutation.yml`) that reports and uploads the HTML report; it is **not** part of `pnpm validate` and **never blocks merge** (Stryker's `break` threshold is unset), so it adds no merge flakiness. `thresholds.low` is the achieved baseline floor, so a regression shows in the report. When a run surfaces a clearly-shallow survivor in these packages, strengthen the assertion (don't chase a specific score, and don't waste effort on equivalent mutants). To extend coverage to another package later, add its `src/**/*.ts` to the `mutate` globs (server/web are out of scope for v0 — slower and I/O-bound).
