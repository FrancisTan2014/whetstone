...
test("authoring a saved reader note's first card enrolls it, persists, and survives a later body edit", async ({
  ...
  editor = page.getByRole("dialog", EDIT_DIALOG);
  await editor.getByRole("tab", { name: "Cards" }).click();
  cardsPanel = editor.getByRole("tabpanel", { name: "Cards" });
  await expect(cardsPanel.getByRole("button", { name: /Alpha/ })).toBeVisible();
  await expect(cardsPanel.getByText("Due now")).toBeVisible();

  // Editing the note body later must NOT reset the schedule: after saving and closing, reopening the note
  // still shows the card due now (the shared card was never rescheduled by the edit).
  await editor.getByRole("tab", { name: "Note" }).click();
