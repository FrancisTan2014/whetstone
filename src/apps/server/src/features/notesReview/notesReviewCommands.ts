...
// The outcome of rating one prompt: the rescheduled card's next FSRS state, or `not_found` when the prompt
// is not the caller's or carries no card (paused/unenrolled) — the route maps that to 404.
export type RateNotePromptResult =
  | Readonly<{ review: NoteReviewRatingResultDto["review"]; status: "rated" }>
...
// card is rescheduled; the returned state's `due` is the next scheduled date the session shows. A prompt
...
export async function rateNotePrompt(
  ...
  return { review: result.state, status: "rated" };
}
