...
try {
  ...
  const wiktionaryLookup = createWiktionaryEntryLookup(
    createFreeDictionaryProvider({ httpClient })
  );
  ...
  lookupSources.push({
    ...
    languages: ["zh-CN", "zh-TW"],
    lookup: (term, options) =>
      explain({ context: options.context, language: options.language, term })
  });
  ...
  const server = createServer({
    ...
    pdfImport: {
      ...
      beginReview: (userId, attemptId) =>
        beginPdfReview(workCreationDependencies, userId, attemptId)
    },
    readingPosition: { db },
    preferences: { db },
    recitation: {
