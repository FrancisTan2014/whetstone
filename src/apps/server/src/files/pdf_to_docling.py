...
def build_range_payload(
    ...
    When the caller supplies the document's already-projected bookmark records, attaches them as
    ``outline`` (#815's declared heading hierarchy) — document-level evidence repeated on every range,
    exactly like ``metadata``, so the Node side takes the first non-empty one.
    """
