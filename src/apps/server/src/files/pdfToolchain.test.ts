...
describe("classifyOcrError", () => {
  ...
  it("treats an error without a usable `code` as an ordinary failure (not a toolchain gap)", () => {
    ...
    // OCR/input failures, not missing-toolchain signals.
    const noCode = new Error("some ocr error");
    expect(classifyOcrError(noCode)).toBe(noCode);

    const oddCode = classifyOcrError({ code: { nested: true } });
