...
//
// Usage:
//   node scripts/delivery/testerNextAction.mjs
//
...
try {
  ...
  console.error(`testerNextAction: failed to query GitHub: ${err.message}`);
  process.exit(1);
}
...
if (budget === 0) {
  console.error(
    `testerNextAction: ${openBugs} open [Bug]s >= cap ${OPEN_BUG_CAP}; idle (let the developer clear the backlog).`
  );
  console.log("idle");
  process.exit(0);
}
...
console.error(
  `testerNextAction: ${openBugs} open [Bug]s; budget=${budget} (open-bug cap ${OPEN_BUG_CAP}, per-run cap ${PER_RUN_CAP}).`
);
