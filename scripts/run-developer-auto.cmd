...
set "PROMPT=Run automatically per your agent instructions (## Run automatically): schedule a self-paced foreground loop with Copilot's scheduled-task feature, and on each tick decide with `node scripts/developer-next-action.mjs` then complete exactly one unit of work (fix the changes-requested PR and push, or implement the next ready issue and open a PR; for wait/idle do nothing), then re-arm the schedule to run the next tick in about 10 minutes (600s). Stay in the foreground; never detach. Do the first tick now."
